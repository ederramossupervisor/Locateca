...
const Livros = (() => {
  ...
  function preencherFormulario(book) {
    ...
    if (container) {
      atualizarEstrelasVisual(container, 0);
    }
  ...
  function preencherFormularioCompleto(livro) {
    ...
    document.getElementById('classico').checked =
      livro.Clássico === 'true' || livro.Clássico === true;
    ...
    // Nota antiga (0-10) é convertida para 0-5
    const notaOriginal = Number(livro.Nota) || 0;

    notaSelecionada = Math.min(
      5,
      Math.max(0, Math.floor(notaOriginal / 2))
    );

    const container = document.getElementById('nota-estrelas-container');

    if (container) {
      atualizarEstrelasVisual(container, notaSelecionada);
    }
    ...
    document.getElementById('urlCapa').value =
      livro.URLCapa || livro.ImagemCapa || '';

    document.getElementById('paginasLidas').value =
      livro.PáginasLidas || 0;

    document.getElementById('paginasAnosAnteriores').value =
      livro.PaginasAnosAnteriores || 0;

    document.getElementById('favorito').checked =
      livro.Favorito === 'true' || livro.Favorito === true;
  ...
  async function salvarLivro(e) {
    ...
    const book = {
      ...
      observacoes:
        document.getElementById('observacoes').value,
      urlCapa:
        Util.converterLinkDrive(urlCapa.value),
      paginasLidas:
        Number(document.getElementById('paginasLidas').value) || 0,
      dataInicio:
        document.getElementById('data-inicio').value,
      paginasAnosAnteriores:
        Number(document.getElementById('paginasAnosAnteriores').value) || 0,
      dataTermino:
        document.getElementById('data-termino').value
    };
    ...
    try {
      ...
      if (resposta && resposta.status === 'ok') {
        Util.toast(
          isEdicao ? 'Livro atualizado!' : 'Livro adicionado!',
          'success'
        );
  ...
  function limparFormulario() {
    ...
    const container =
      document.getElementById('nota-estrelas-container');
  ...
  function voltarParaBiblioteca() {
    const link =
      document.querySelector('.nav-link[data-page="biblioteca"]');
    ...
    if (link) {
      ...
      const pageBiblioteca =
        document.getElementById('page-biblioteca');

      if (pageBiblioteca) {
        pageBiblioteca.classList.add('active');

        if (
          typeof Biblioteca !== 'undefined' &&
          Biblioteca.init
        ) {
          Biblioteca.init();
        }
  ...
  function editarLivro(livro) {
    ...
    if (!formEl) {
      Util.toast(
        'Erro: formulário de livro não encontrado.',
        'danger'
      );
    ...
    document
      .querySelectorAll('.nav-link[data-page="adicionar"]')
      .forEach(l => l.click());
  ...
  function criarBotaoCancelarEdicao() {
    if (
      ...
      btnCancel.className =
        'btn btn-outline-warning ms-2 d-none';
      ...
      const botoesDiv =
        document.querySelector('#book-form .d-flex');
